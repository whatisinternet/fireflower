...
describe Case do
  ...
  context 'pascal case' do
    describe 'setup' do
      ...
      xit 'to_pascal_case accepts an input' do
    ...
    describe 'full functionality' do
      ...
      xit 'returns a pascal case string if given one' do
        expect(@case.to_pascal_case("foo_bar")).to eq("FooBar")
      end

      xit 'returns a pascal case string if given camel case' do
        expect(@case.to_pascal_case("fooBar")).to eq("FooBar")
      end
    end

    describe 'benchmarks' do
      xit 'performs the pascal in Rust at least 30 times faster than Ruby' do
        expect {
